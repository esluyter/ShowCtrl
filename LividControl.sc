LividControl {
  ...
  var <>numBanks /* TODO update states when this changes */, <currentBank = 0, <>bankChangeButton;
  ...
  *new { |deviceName, portName, numBanks = 4, makeDef = true|
  ...
  /* Booze beer buzz buddy */

  currentBank_ { |newCurrentBank|
    if (outDevice.notNil) {
      if (newCurrentBank.isInteger) {
        ...
        if (customBanks[newCurrentBank].notNil) {
          "Custom bank".postln;
          currentBank = newCurrentBank;
          (1..32).do { |i|
            var knobNum = customBanks[currentBank][i];
            if (knobNum.isNil) {
              outDevice.control(inChannel, i, 0);
            } {
              if (knobNum.class == Association) {
                outDevice.control(inChannel, i, knobStates[knobNum.key][knobNum.value - 1]);
              } {
                outDevice.control(inChannel, i, knobStates[0][knobNum - 1]);
              };
            };
          };
          if (bankChangeButton.notNil) {
  ...
  setCustomBank { |name ...pairs|
    var bank;
    name = name.asSymbol;
    bank = customBanks[name];
    if (bank.isNil) {
      bank = ();
      customBanks[name] = bank;
    };

    pairs.pairsDo { |index, target|
      bank[index] = target;
    };

    this.changed(\customBanks);
  }

  customBankNames {
    ^customBanks.keys.asArray.sort;
  }
